...
namespace SharedMaterials {
    ...
    inline void BuildAllMaterials() {
        ...
        // Elements
        G4Element* elH  = nist->FindOrBuildElement("H"); G4Element* elC  = nist->FindOrBuildElement("C");
        G4Element* elN  = nist->FindOrBuildElement("N"); G4Element* elO  = nist->FindOrBuildElement("O");
        G4Element* elF  = nist->FindOrBuildElement("F"); G4Element* elMg = nist->FindOrBuildElement("Mg");
        G4Element* elAl = nist->FindOrBuildElement("Al"); G4Element* elSi = nist->FindOrBuildElement("Si");
        G4Element* elP  = nist->FindOrBuildElement("P"); G4Element* elCl = nist->FindOrBuildElement("Cl");
        G4Element* elCa = nist->FindOrBuildElement("Ca"); G4Element* elMn = nist->FindOrBuildElement("Mn");
        G4Element* elGa = nist->FindOrBuildElement("Ga"); G4Element* elGe = nist->FindOrBuildElement("Ge");
        G4Element* elBr = nist->FindOrBuildElement("Br"); G4Element* elLa = nist->FindOrBuildElement("La");
        G4Element* elCe = nist->FindOrBuildElement("Ce"); G4Element* elGd = nist->FindOrBuildElement("Gd");
        G4Element* elBi = nist->FindOrBuildElement("Bi"); G4Element* elW  = nist->FindOrBuildElement("W");

        // Base NIST Materials
        G4Material* matWater = nist->FindOrBuildMaterial("G4_WATER");
        G4Material* matCa    = nist->FindOrBuildMaterial("G4_Ca");

        // Scintillators & Basics
        G4Material* teflon = new G4Material("teflon", 2.2*g/cm3, 2);
        teflon->AddElement(elC, 0.240183); teflon->AddElement(elF, 0.759817);
        ...
        cebr3->AddElement(elCe, 0.3714); cebr3->AddElement(elBr, 0.6286);

        G4Material* bgo = new G4Material("BGO", 7.12*g/cm3, 3);
        bgo->AddElement(elBi, 0.671); bgo->AddElement(elGe, 0.175); bgo->AddElement(elO, 0.154);

        G4Material* gaggce = new G4Material("GAGGCe", 6.63*g/cm3, 5);
        gaggce->AddElement(elGd, 0.5032); gaggce->AddElement(elAl, 0.0577);
        gaggce->AddElement(elGa, 0.2237); gaggce->AddElement(elO, 0.2053); gaggce->AddElement(elCe, 0.0100);

        G4Material* labr3ce = new G4Material("LaBr3Ce", 5.08*g/cm3, 3);
        labr3ce->AddElement(elLa, 0.3650); labr3ce->AddElement(elBr, 0.6300); labr3ce->AddElement(elCe, 0.0050);

        G4Material* polystyrene = new G4Material("polystyrene", 1.05*g/cm3, 2);
        polystyrene->AddElement(elC, 0.922582); polystyrene->AddElement(elH, 0.077418); // CH approximation

        G4Material* cao = new G4Material("calcium_oxide", 1.22*g/cm3, 2);
        cao->AddElement(elCa, 0.7147); cao->AddElement(elO, 0.2853);
        ...
        auto buildMix = [&](const G4String& name, G4double density, G4Material* m1, G4double f1, G4Material* m2, G4double f2) {
            ...
            if (f1 > 0) mat->AddMaterial(m1, f1); if (f2 > 0) mat->AddMaterial(m2, f2);
        };
        ...
        auto buildWaCa = [&](const G4String& name, G4double density, G4double fW, G4double fCa) {
            ...
            if (fW > 0) mat->AddMaterial(matWater, fW); if (fCa > 0) mat->AddMaterial(matCa, fCa);
        };
        ...
        ln300->AddElement(elH, 0.08460); ln300->AddElement(elC, 0.59370); ln300->AddElement(elN, 0.01960);
        ln300->AddElement(elO, 0.18140); ln300->AddElement(elMg, 0.11190); ln300->AddElement(elSi, 0.00780); ln300->AddElement(elCl, 0.00100);

        G4Material* ln450 = new G4Material("LN_450_lung", 0.428*g/cm3, 7);
        ln450->AddElement(elH, 0.08470); ln450->AddElement(elC, 0.59560); ln450->AddElement(elN, 0.01970);
        ln450->AddElement(elO, 0.18110); ln450->AddElement(elMg, 0.11210); ln450->AddElement(elSi, 0.00580); ln450->AddElement(elCl, 0.00100);
